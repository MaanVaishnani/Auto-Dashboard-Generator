...
def load_file(file_path: str) -> pd.DataFrame:
    ...
    try:
        if ext == ".csv":
            ...
            # Safety net: if detection still resulted in a single column,
            # explicitly try each other known delimiter and use the first one
            # that produces multiple, consistently-shaped columns. We do NOT
            # use pandas' unrestricted sep=None sniffer here, since it can
            # mistake any character (even a letter inside a word) for a
            # delimiter on genuinely single-column files.
            if df.shape[1] == 1:
                for candidate in [",", ";", "\t", "|"]:
                    if candidate == delimiter:
                        continue
                    try:
                        df_retry = pd.read_csv(file_path, sep=candidate)
...
# ---------------------------------------------------------------------
def _try_parse_as_numeric(series: pd.Series):
    """
    Attempts to coerce a text column that LOOKS numeric but contains
    formatting like currency symbols, thousands separators, or percent
    signs (e.g. "$55,000.00", "1,200", "45%") into a true numeric series.

    Returns the converted numeric series if at least 70% of non-null values
    parse successfully, otherwise returns None (leave the column as-is).
    """
    ...
    # Strip common currency symbols, thousands separators, and percent signs
    cleaned = cleaned.str.replace(r"[₹$€£,%]", "", regex=True)
    cleaned = cleaned.str.replace(r"^\((.*)\)$", r"-\1", regex=True)  # accounting negatives like (500)
    ...
    return converted if success_rate >= 0.7 else None
...
def detect_column_types(df: pd.DataFrame) -> dict:
    ...
    for col in df.columns:
        ...
        if is_text_like:
            ...
            # Try numeric detection for formatted numbers stored as text,
            # e.g. "$55,000.00", "1,200", "45%" - common in real-world exports
            if _try_parse_as_numeric(series) is not None:
...
def clean_data(df: pd.DataFrame, column_types: dict) -> tuple[pd.DataFrame, dict]:
    ...
    # --- Convert formatted-number text columns (e.g. "$55,000.00") to real numeric ---
    numeric_format_fixed = []
    for col, ctype in column_types.items():
        if ctype == "numeric" and col in df_clean.columns:
            is_text_like = pd.api.types.is_object_dtype(df_clean[col]) or pd.api.types.is_string_dtype(df_clean[col])
            if is_text_like:
                converted = _try_parse_as_numeric(df_clean[col])
                if converted is not None:
                    df_clean[col] = converted
                    numeric_format_fixed.append(col)
    summary["numeric_format_fixed"] = numeric_format_fixed
